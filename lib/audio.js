...
// Global variables for 40Hz Clicker
let clickerAudioContext;
let clickerSource, clickerGainNode;
let clickerVolumeSlider = null;
let isClickerPlaying = false;
const CLICKER_MAX_GAIN = 0.5;
const CLICKER_DEFAULT_GAIN = 0.3;
...
// Global variables for stream metadata (now-playing)
...
const MARQUEE_WIDTH = 30; // characters visible in tab
const METADATA_POLL_INTERVAL = 15000; // 15 seconds
const MARQUEE_TICK_INTERVAL = 300; // milliseconds
...
function createVolumeSlider(
  ...
  makeDraggable(sliderContainer, { allowFrom: ".audio-slider-label" });
...
// Stream metadata and now-playing functions

const METADATA_SOURCES = {
  // Radio Swiss: https://api.radioswissjazz.ch/api/v1/rsj/en/current
  radioswiss: {
    delayMs: 0,
    urlFor: (id) => id, // full URL passed directly
    extract: (data, url) => {
      const m = data.channel?.playingnow?.current?.metadata;
      if (!m) return null;
      const station = (url.match(/\/(rsj|rsp|rsc)/) || [])[1];
      const coverUrl =
        m.coverId && station
          ? `https://cdne-satr-prd-${station}-covers.azureedge.net/200/${m.coverId}.jpg`
          : null;
      return { title: m.title || "", artist: m.artist || "", coverUrl };
    },
  },
  // Antenne family: https://www.rockantenne.de/api/metadata/now/{mountpoint}
  antenne: {
    delayMs: 30000,
    urlFor: (id) => `https://www.rockantenne.de/api/metadata/now/${id}`,
    extract: (data) => {
      const d = data.data;
      if (!d?.artist && !d?.title) return null;
      const cover = d.cover;
      const coverUrl = cover
        ? `${cover.baseurl}200x200/${cover.filename}.webp`
        : null;
      return { title: d.title || "", artist: d.artist || "", coverUrl };
    },
  },
  // laut.fm: https://api.laut.fm/station/{stationname}/current_song
  laut: {
    delayMs: 0,
    urlFor: (id) => `https://api.laut.fm/station/${id}/current_song`,
    extract: (data) => {
      if (!data.title && !data.artist) return null;
      return { title: data.title || "", artist: data.artist?.name || "" };
    },
  },
};

function resolveMetadataSource(metadataUrl) {
  const colonIdx = metadataUrl.indexOf(":");
  if (colonIdx !== -1) {
    const prefix = metadataUrl.substring(0, colonIdx);
    const id = metadataUrl.substring(colonIdx + 1);
    if (METADATA_SOURCES[prefix]) {
      return {
        source: METADATA_SOURCES[prefix],
        url: METADATA_SOURCES[prefix].urlFor(id),
      };
    }
  }
  // Bare URL — use radioswiss extractor (legacy behaviour)
  return { source: METADATA_SOURCES.radioswiss, url: metadataUrl };
}

async function fetchStreamMetadata(metadataUrl) {
  try {
    const { source, url } = resolveMetadataSource(metadataUrl);
    const response = await fetch(url);
    const data = await response.json();
    const track = source.extract(data, url);
    if (
      track &&
      (track.title !== currentTrackInfo.title ||
        track.artist !== currentTrackInfo.artist)
    ) {
      const apply = () => {
        ...
        updateTabTitleMarquee();
      };
      const isFirstLoad = !currentTrackInfo.title && !currentTrackInfo.artist;
      const alreadyPending =
        pendingTrackInfo?.title === track.title &&
        pendingTrackInfo?.artist === track.artist;
      if (source.delayMs > 0 && !isFirstLoad && !alreadyPending) {
        if (metadataDelayTimer) clearTimeout(metadataDelayTimer);
        pendingTrackInfo = track;
        metadataDelayTimer = setTimeout(apply, source.delayMs);
      } else if (!alreadyPending) {
        apply();
      }
    }
  } catch (e) {
    console.warn("Failed to fetch stream metadata:", e);
  }
}

function updateTabTitleMarquee() {
  if (!currentTrackInfo.title && !currentTrackInfo.artist) return;
  const parts = [];
  if (currentTrackInfo.artist) parts.push(currentTrackInfo.artist);
  if (currentTrackInfo.title) parts.push(currentTrackInfo.title);
...
function tickMarquee() {
  if (!marqueeText || !isStreamPlaying || !marqueeEnabled) {
    if (!isStreamPlaying) {
      document.title = "NT";
    }
    return;
  }
  const display = (marqueeText + marqueeText).substring(
    marqueePosition,
    marqueePosition + MARQUEE_WIDTH,
  );
  document.title = display;
  marqueePosition = (marqueePosition + 1) % marqueeText.length;
}

function createNowPlayingDisplay() {
  // Add now-playing info to the stream slider container, below the label
  const slider = document.getElementById("stream-slider");
  if (!slider) return null;

  let display = slider.querySelector(".now-playing-info");
  if (display) return display;

  // Create now-playing section
  display = document.createElement("div");
  display.className = "now-playing-info";
  display.innerHTML = `
    <img class="now-playing-cover" src="" alt="" hidden>
  ...
  // Insert after the label, before the track
  const label = slider.querySelector(".audio-slider-label");
  const track = slider.querySelector(".audio-slider-track");
  if (label && track) {
    slider.insertBefore(display, track);
  } else {
    slider.appendChild(display);
  }

  // Wrap track in a collapsible container and add toggle
  if (track && !slider.querySelector(".volume-section")) {
    const volumeSection = document.createElement("div");
    volumeSection.className = "volume-section collapsed";

    const toggle = document.createElement("i");
    toggle.className = "volume-toggle ph-light ph-speaker-simple-low";
    toggle.addEventListener("click", () => {
      volumeSection.classList.toggle("collapsed");
    });

    // Move track into volume section
    track.parentNode.insertBefore(volumeSection, track);
    volumeSection.appendChild(toggle);
    volumeSection.appendChild(track);
  }
...
function updateNowPlayingDisplay() {
  const slider = document.getElementById("stream-slider");
  if (!slider) return;
  const display = slider.querySelector(".now-playing-info");
  if (!display) return;

  const titleEl = display.querySelector(".now-playing-title");
  const artistEl = display.querySelector(".now-playing-artist");
  const coverEl = display.querySelector(".now-playing-cover");

  setMarqueeText(titleEl, currentTrackInfo.title);
  setMarqueeText(artistEl, currentTrackInfo.artist);

  if (coverEl) {
    if (currentTrackInfo.coverUrl) {
      coverEl.src = currentTrackInfo.coverUrl;
...
function setMarqueeText(el, text) {
  ...
  // Check if text overflows after render
...
function removeNowPlayingDisplay() {
  const slider = document.getElementById("stream-slider");
  if (!slider) return;
  const display = slider.querySelector(".now-playing-info");
  if (display) display.remove();

  // Also remove volume section wrapper and restore track
  const volumeSection = slider.querySelector(".volume-section");
  if (volumeSection) {
    const track = volumeSection.querySelector(".audio-slider-track");
    if (track) {
...
function startSseMetadata(station, noMarquee) {
  if (sseEventSource) {
    sseEventSource.close();
    sseEventSource = null;
  }
  createNowPlayingDisplay();
  sseEventSource = new EventSource("https://nightride.fm/meta");
  sseEventSource.onmessage = (event) => {
    try {
      const data = JSON.parse(event.data);
      const stationData = Array.isArray(data)
        ? data.find((s) => s.station === station)
        : null;
      if (stationData) {
        const newTitle = stationData.title || "";
        const newArtist = stationData.artist || "";
        if (
          newTitle !== currentTrackInfo.title ||
          newArtist !== currentTrackInfo.artist
        ) {
          currentTrackInfo = { title: newTitle, artist: newArtist };
          updateNowPlayingDisplay();
          updateTabTitleMarquee();
        }
      }
    } catch (e) {
      // keepalive or non-JSON message, ignore
    }
  };
  sseEventSource.onerror = (e) => {
    console.warn("Nightride SSE error:", e);
  };
  if (!noMarquee) {
    marqueeIntervalId = setInterval(tickMarquee, MARQUEE_TICK_INTERVAL);
  }
}

function startMetadataPolling(metadataUrl, noMarquee) {
  // Clear any existing intervals
  stopMetadataPolling();

  currentMetadataUrl = metadataUrl;
  marqueeEnabled = !noMarquee;

  if (metadataUrl.startsWith("sse:")) {
    startSseMetadata(metadataUrl.substring(4), noMarquee);
    return;
  }
  ...
  // Fetch immediately
  fetchStreamMetadata(metadataUrl);
  ...
  // Set up marquee ticker if enabled
  if (marqueeEnabled) {
    marqueeIntervalId = setInterval(tickMarquee, MARQUEE_TICK_INTERVAL);
  }
}
...
function stopMetadataPolling() {
  ...
  marqueeText = "";
  marqueePosition = 0;
  removeNowPlayingDisplay();
}

async function playStream(props = {}) {
  const { streamUrl, title, metadataUrl, noMarquee } = props;
  ...
  try {
    ...
    // Start metadata polling if a metadata URL is provided
    if (metadataUrl) {
      startMetadataPolling(metadataUrl, noMarquee);
    }
  } catch (err) {
...
function stopStream() {
  ...
  // Clean up metadata polling and reset title
  stopMetadataPolling();
  document.title = "NT";
}
...
function handleHashChange() {
  ...
  if (window.location.hash.startsWith("#stream:")) {
    ...
    if (parts.length >= 2) {
      ...
      const metadataUrl =
        parts.length > 3 ? decodeURIComponent(parts[3]) : undefined;
      const noMarquee = parts.length > 4 && parts[4] === "noMarquee";
      playStream({ streamUrl, title, metadataUrl, noMarquee });
      called = true;
    }
  }
  ...
  if (called) {
    // Only set "NT 🔊" if no metadata polling (marquee will handle title)
    if (!metadataIntervalId) {
      document.title = "NT 🔊";
    }
    window.history.replaceState(null, null, " ");
    // Clear link highlights
    if (typeof window.hasKeys !== "undefined") {
      window.hasKeys = "";
    }
    ...
    document
      .querySelectorAll(".quicklink.shortcut .highlight")
      .forEach((span) => {
...
function startClicker() {
  if (clickerVolumeSlider && isClickerPlaying) {
    stopClicker();
    return;
  }

  let currentGainSetting = CLICKER_DEFAULT_GAIN;
  if (clickerVolumeSlider && clickerVolumeSlider.knob) {
    const knobY = parseFloat(clickerVolumeSlider.knob.style.top);
    const { trackHeight, knobHeight, maxGain } = clickerVolumeSlider;
    if (!isNaN(knobY) && trackHeight != null && knobHeight != null && maxGain != null) {
      const gainFraction = 1 - knobY / (trackHeight - knobHeight);
      currentGainSetting = Math.max(0, Math.min(gainFraction * maxGain, maxGain));
    }
  } else {
    currentGainSetting = Math.min(currentGainSetting, CLICKER_MAX_GAIN);
  }

  if (!clickerAudioContext || clickerAudioContext.state === "closed") {
    clickerAudioContext = new (window.AudioContext || window.webkitAudioContext)();
    
    const sampleRate = clickerAudioContext.sampleRate;
    const hz = 40; // 40Hz frequency
    // Calculate how many samples long one cycle of 40Hz is (e.g. 44100 / 40 = 1102.5)
    const cycleLengthSamples = Math.round(sampleRate / hz); 
    
    const buffer = clickerAudioContext.createBuffer(1, cycleLengthSamples, sampleRate);
    const output = buffer.getChannelData(0);
    
    // Create a sharp 2-millisecond "click" at the start of the buffer
    const clickDurationSamples = Math.round(sampleRate * 0.002); 
    
    const toneFrequency = 150; // A low, soft bass tone instead of static
    for (let i = 0; i < cycleLengthSamples; i++) {
      if (i < clickDurationSamples) {
        // Smooth sine wave burst
        const decay = Math.exp(-i / (clickDurationSamples / 3));
        output[i] = Math.sin((i / sampleRate) * Math.PI * 2 * toneFrequency) * decay;
      } else {
        output[i] = 0;
      }
    }

    clickerSource = clickerAudioContext.createBufferSource();
    clickerSource.buffer = buffer;
    clickerSource.loop = true; // Loop the buffer to create the continuous click train

    clickerGainNode = clickerAudioContext.createGain();
    clickerSource.connect(clickerGainNode);
    clickerGainNode.connect(clickerAudioContext.destination);
    
    clickerSource.start();
  }

  clickerGainNode.gain.setValueAtTime(currentGainSetting, clickerAudioContext.currentTime);

  if (!clickerVolumeSlider) {
    clickerVolumeSlider = createVolumeSlider(
      "clicker-slider",
      "40Hz Clicks",
      currentGainSetting,
      CLICKER_MAX_GAIN,
      (newGain) => {
        if (clickerGainNode && clickerAudioContext && clickerAudioContext.state === "running") {
          clickerGainNode.gain.setValueAtTime(newGain, clickerAudioContext.currentTime);
        }
      }
    );
  } else {
    updateSliderPosition(clickerVolumeSlider, currentGainSetting);
  }

  if (clickerAudioContext.state === "suspended") clickerAudioContext.resume();
  isClickerPlaying = true;
  console.info(`Playing 40Hz Clicker. Gain: ${currentGainSetting.toFixed(2)}`);
}

function stopClicker() {
  if (isClickerPlaying && clickerAudioContext) {
    clickerAudioContext.close().then(() => {
      clickerAudioContext = null;
      clickerSource = null;
      clickerGainNode = null;
    });
    isClickerPlaying = false;
    console.info("40Hz Clicker stopped.");
  }
}

if (window.location.hash === "#startClicker") {
    startClicker();
    called = true;
  }
