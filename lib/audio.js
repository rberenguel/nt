...
const METADATA_SOURCES = {
  ...
  radioswiss: {
    ...
    extract: (data, url) => {
      ...
      const coverUrl = m.coverId && station
...
function resolveMetadataSource(metadataUrl) {
  ...
  if (colonIdx !== -1) {
    ...
    if (METADATA_SOURCES[prefix]) {
      return { source: METADATA_SOURCES[prefix], url: METADATA_SOURCES[prefix].urlFor(id) };
    }
  }
...
async function fetchStreamMetadata(metadataUrl) {
  try {
    ...
    if (track && (track.title !== currentTrackInfo.title || track.artist !== currentTrackInfo.artist)) {
        const apply = () => {
      ...
      const alreadyPending = pendingTrackInfo?.title === track.title && pendingTrackInfo?.artist === track.artist;
