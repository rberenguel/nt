...
function bindBlinkerEvents(container) {
  ...
  const setUI = (enabled) => {
    ...
    els.connect.style.background = enabled
      ? "var(--light-grey)"
      : "var(--cyan)";
  ...
  els.connect.addEventListener("click", async () => {
    ...
    try {
      ...
      blinkerDevice = await navigator.bluetooth.requestDevice({
        filters: [{ services: [SERVICE_UUID] }],
      });
      blinkerDevice.addEventListener("gattserverdisconnected", () =>
        setUI(false),
      );
  ...
  els.start.addEventListener("click", async () => {
    ...
    if (blinkerTimerChar)
      await blinkerTimerChar.writeValue(
        new Uint8Array([TIMER_STEPS[timerStepIdx]]),
      );
    if (blinkerBrightChar)
      await blinkerBrightChar.writeValue(
        new Uint8Array([BRIGHT_BLE[brightStepIdx]]),
      );
  ...
  els.timeNext.addEventListener("click", async () => {
    if (timerStepIdx < TIMER_STEPS.length - 1) {
      timerStepIdx++;
      await updateTimer();
    }
  });
  ...
  els.brightNext.addEventListener("click", async () => {
    if (brightStepIdx < BRIGHT_STEPS.length - 1) {
      brightStepIdx++;
      await updateBright();
    }
  });
