...
function processReplacements(replacementItems) {
  ...
  if (!replacements) {
    console.warn("No replacement items found.");
    return;
  }
  if (
  ...
  if (replacementKeys.length > 0) {
    console.info("Calling replacementsFromMarkdown for keys:", replacementKeys);
    replacementsFromMarkdown(replacementKeys, (b) => {
      window._ntCommands = window._ntCommands || [];
      window._ntCommands = window._ntCommands.concat(b);
    });
  } else {
    console.warn("No actual replacement keys found in replacement item.");
  }
}
