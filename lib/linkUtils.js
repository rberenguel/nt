...
function addLinksToDiv(links, targetDivId) {
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    if (link.kind == "settings") {
      ...
      const value = link.display.split(":").slice(1).join(":").trim();
      if (property === "edit") {
        console.log("edit property");
        console.log(value);
        const a = document.createElement("A");
        a.classList.add("edit-link-section");
        a.classList.add("iconoir-tools");
        wrapperNode.appendChild(a);
        a.textContent = "";
        a.href = value;
        continue;
      }
    ...
    let node;
    if (link.url) {
      if (link.url.startsWith("chrome-extension")) {
        node = document.createElement("A");
        node.addEventListener("click", () => {
          const [extensionId, message] = link.url
    ...
    if (link.foldable) {
      node.addEventListener("click", () => {
        ...
        const siblings = [];
        let currentNode = node.parentNode.nextElementSibling;
        while (currentNode) {
        ...
        Array.from(siblings).map((s) => s.classList.toggle("folded"));
      });
    }
    node.innerHTML = link.display;
    if (link.display.endsWith("<") && link.foldable) {
      node.textContent = node.textContent.replace(/<$/, ">");
      clickers.push(node);
    }
    ...
    targetDiv.classList.add("started");
  }

  if (targetDivId === "center") {
    makeDraggable(targetDiv, { baseTransform: "translate(-50%, -50%)" });
  }
  ...
  document.addEventListener("keydown", function (event) {
    ...
    if (document.getElementById("metap-modal")?.style.display === "block") {
      return;
    }
    ...
    if (window.hasKeys) {
      ...
      if (matchingLink) {
        ...
        if (matchingLink.url.startsWith("chrome-extension")) {
          const [extensionId, message] = matchingLink.url
            .replace("chrome-extension:", "")
            .split(":");
          console.info(`Sending ${message} to extension ${extensionId}`);
          chrome.runtime.sendMessage(extensionId, { message: message });
  ...
  targetDiv.focus();
  clickers.map((c) => c.click());
  setTimeout(() => {
    Array.from(document.querySelectorAll(".quicklink-div")).map((q) =>
      q.classList.add("ql-transition"),
    );
  }, 100);
}

function processLinks(linkItems) {
  console.info(`HANDLER: processLinks called with ${linkItems.length} item(s)`);
  if (!linkItems || linkItems.length === 0) return;
  if (
    typeof linksFromMarkdown !== "function" ||
    typeof addLinksToDiv !== "function"
  )
    return console.error(
      "Dependency check failed inside processLinks: 'linksFromMarkdown' or 'addLinksToDiv'.",
    );
  for (const links of linkItems) {
    const destination = links.div;
    if (!destination) {
      console.warn("Skipping link block: missing 'div' property.", links);
      continue;
    }
    const linkKeys = Object.keys(links).filter(
      (k) => k !== "kind" && k !== "title" && k !== "div",
    );
    if (linkKeys.length > 0) {
      console.info(
        `Calling linksFromMarkdown for destination '${destination}' with keys:`,
        linkKeys,
      );
      linksFromMarkdown(linkKeys, (linkList) =>
        addLinksToDiv(linkList, destination),
      );
    } else {
      console.warn(
        `No actual link keys found in link block for destination '${destination}'.`,
      );
    }
  }
}
