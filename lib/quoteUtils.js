...
const renderQuotes = (opts, randomIndex) => {
  const _quotes = opts.quotes;
  const settings = getSettings(_quotes);
  const targetDivId = opts.target;
  const quotes = settings.concat(_quotes[randomIndex]); // Just place them at the beginning and call it a day
...
function addQuotesToDiv(opts = {}) {
  const today = opts.today ?? false;
  let randomIndex = Math.floor(Math.random() * _quotes.length);
  if (today) {
    getDailyRandom(_quotes.length, (idx) => renderQuotes(opts, idx));
  } else {
    renderQuotes(opts, randomIndex);
  }
}

function processQuotes(quoteItems) {
  console.log(
    `HANDLER: processQuotes called with ${quoteItems.length} item(s)`,
  );
  // ... (rest of function unchanged, includes qfm) ...
  if (!quoteItems || quoteItems.length === 0) return;
  if (
    typeof quotesFromMarkdown !== "function" ||
    typeof addQuotesToDiv !== "function"
  )
    return console.error(
      "Dependency check failed inside processQuotes: 'quotesFromMarkdown' or 'addQuotesToDiv'.",
    );
  const qfm = (files, target, today) => {
    quotesFromMarkdown(
      files,
      (ts) => {
        addQuotesToDiv({ quotes: ts, target: target, today: today });
      },
      "",
    );
  };
  for (const quotes of quoteItems) {
    const target = quotes.div;
    const today = quotes.today !== "false";
    if (!target) {
      console.warn("Skipping quote block: missing 'div' property.", quotes);
      continue;
    }
    const quoteKeys = Object.keys(quotes).filter(
      (k) => k !== "kind" && k !== "title" && k !== "div" && k !== "today",
    );
    if (quoteKeys.length > 0) {
      console.log(
        `Calling qfm (quotesFromMarkdown) for target '${target}' with files:`,
        quoteKeys,
      );
      qfm(quoteKeys, target, today);
    } else {
      console.log(
        `No actual quote file keys found in quote block for target '${target}'.`,
      );
    }
  }
}
