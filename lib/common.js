...
// Apply zen mode synchronously before any widgets are created, to avoid flash
...
function getZenMode() {
  ...
  return Promise.resolve(localStorage.getItem("zenMode") === "true");
}

function setZenMode(enabled) {
  localStorage.setItem("zenMode", enabled ? "true" : "false");
...
function applyZenMode(enabled) {
  const contentDivs = document.querySelectorAll(
    "#upper-left, #upper-right, #lower-left, #lower-right, #center, .iframe-wrapper, .post-it, .eight-ball-wrapper",
  );
...
function makeDraggable(el, { allowFrom = null, baseTransform = "" } = {}) {
  const opts = {
    listeners: {
      move(event) {
        ...
        target.style.transform = baseTransform
          ? `${baseTransform} translate(${x}px, ${y}px)`
          : `translate(${x}px, ${y}px)`;
  ...
  if (allowFrom) opts.allowFrom = allowFrom;
  interact(el)
    .draggable(opts)
    .on("dragstart", (event) => {
